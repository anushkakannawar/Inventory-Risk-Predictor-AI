...
interface SKUTableProps {
  ...
  onSelectSKU?: (skuIds: string[]) => void;
  selectedSKUs?: string[];
}
...
export function SKUTable({ skus, analyses, onSelectSKU, selectedSKUs = [] }: SKUTableProps) {
  const toggleSelectAll = (checked: boolean) => {
    if (onSelectSKU) {
      onSelectSKU(checked ? skus.map(s => s.id) : []);
    }
  };

  const toggleSelect = (skuId: string, checked: boolean) => {
    if (onSelectSKU) {
      if (checked) {
        onSelectSKU([...selectedSKUs, skuId]);
      } else {
        onSelectSKU(selectedSKUs.filter(id => id !== skuId));
      }
    }
  };
              ...
              >
                <TableCell>
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => toggleSelect(sku.id, checked as boolean)}
                  />
                </TableCell>
