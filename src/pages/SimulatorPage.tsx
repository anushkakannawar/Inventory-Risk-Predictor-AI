...
export default function SimulatorPage() {
  ...
  const getRiskChange = (current: number, baseline: number) => {
    ...
    if (Math.abs(diff) < 0.1) return { icon: Minus, color: 'text-muted-foreground', text: 'No change' };
    const formattedDiff = Math.abs(diff).toFixed(1);
    if (diff > 0) return { icon: TrendingUp, color: 'text-risk-high', text: `+${formattedDiff}%` };
    return { icon: TrendingDown, color: 'text-risk-low', text: `${formattedDiff}%` };
  };
          ...
          <div className="lg:col-span-2 space-y-6">
            {simulatedAnalysis && baselineAnalysis && (
              <>
                      ...
                      {(['understockRisk', 'overstockRisk', 'deadInventoryRisk'] as const).map((key) => {
                        ...
                        return (
                          <div key={key} className="text-center space-y-4">
                            ...
                            <p className="text-xs text-muted-foreground">
                              Baseline: {baselineAnalysis[key].toFixed(1)}%
                            </p>
                          </div>
                        );
                      })}
                ...
                {/* Projection Chart */}
                <InventoryChart
                  data={simulatedAnalysis.simulationResult}
