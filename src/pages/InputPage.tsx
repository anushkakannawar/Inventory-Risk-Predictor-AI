              ...
              <CardContent className="grid sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between">
                    <Label>Sales Volatility (Volatility: {newSku.salesVariability}%)</Label>
                    <span className="text-sm text-muted-foreground">{newSku.salesVariability}%</span>
                  </div>
                  <Slider
                    value={[newSku.salesVariability]}
                    onValueChange={([val]) => setNewSku({ ...newSku, salesVariability: val })}
                    min={10}
                    max={100}
                    step={5}
                    className="py-2"
                  ...
                  <p className="text-xs text-muted-foreground">
                    Higher volatility means more unpredictable daily sales.
                  </p>
                </div>
                <div className="space-y-2">
                ...
                {skus.map((sku) => (
                  <div key={sku.id} className="flex items-center justify-between py-3">
                    <div>
                      ...
                      {sku.currentInventory} units • {formatCurrency(sku.unitCost)}/unit • {sku.dailySalesRate} sales/day
                    </div>
                    <Button
                      variant="ghost"
                      size="icon"
